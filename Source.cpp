...
//Shared Values
int NUM_ITEMS=100;					//This can actually stay constant after program is running
...
struct ConsumerData
{
	...
	double * ConStoreTotals;			//store totals for each consumer (index based on producer number)
};
ConsumerData * CData;//[NUM_CONSUMERS];			//array to hold ConsumerData structs
...
int main()
{
	...
	StoreTotals = new double[NUM_PRODUCERS];
	...
	for(int p = 0; p < 3; p++)
	{
		...
		for(int c = 0; c < 3; c++)
		{
			...
			ProduceConsume(NUM_PRODUCERS, NUM_CONSUMERS);
...
void ProduceConsume (int NUM_PRODUCERS, int	 NUM_CONSUMERS)
{
	int rc1, rc2;
	...
	CData				=	new ConsumerData[NUM_CONSUMERS];
	//CData->ConStoreTotals = new double[NUM_PRODUCERS];
	...
	for (int t=0; t<NUM_CONSUMERS; t++)
	{
		...
		CData[t].ID=t;
		CData[t].ConStoreTotals = new double[NUM_PRODUCERS];
		rc1=pthread_create(&ConsumerThreads[t], NULL, ConsumerFunction, (void*) &CData[t]);
		if (rc1)
		{
			cout<<"Error, return code from pthread_create is: "<<rc1<<endl;
			exit (-1); //return codes if threads fail to create
		}
	}
...
void * ConsumerFunction (void *t)
{
	...
	ConsumerData * consumer = (ConsumerData*) t;

	cout<<"Consumer: "<<consumer->ID<<" started"<<endl;
	...
	while(1)
	{
		...
		if(TotalConsumed==NUM_ITEMS)		
		{
			...
			cout<<"Consumer: "<<consumer->ID<<" done"<<endl;
			pthread_exit(NULL);				//Exit thread if true
		}
...
this_thread:sleep:(10);
		...
		CData[consumer->ID].ConMonthTotals[Buffer[ReadPosition].getMonth()]	+= Buffer[ReadPosition].getSaleAmount();
		CData[consumer->ID].ConStoreTotals[Buffer[ReadPosition].getStoreID()]	+= Buffer[ReadPosition].getSaleAmount();
